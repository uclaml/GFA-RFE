...
class ReplayBuffer(IterableDataset):
    def __init__(self, storage, max_size, num_workers, nstep, discount,
                 fetch_every, save_snapshot, is_offline, num_episodes):
        ...
        if is_offline:
            for eps_fn in sorted(self._storage._replay_dir.glob('*.npz'), key=lambda x: x.name)[:num_episodes]:
                self._store_episode(eps_fn)
            warn(f'Initialized replay buffer with {self._size} transitions.')
    ...
    def _store_episode(self, eps_fn):
        ...
        while eps_len + self._size > self._max_size:
            warn(f'Replay buffer is full, removing old episodes. {eps_len}, {self._size}, {self._max_size}')
...
def make_replay_loader(storage, max_size, batch_size, num_workers,
                       save_snapshot, nstep, discount, is_offline=False, num_episodes=1000):
    max_size_per_worker = max_size if is_offline else max_size // num_workers
